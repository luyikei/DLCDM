...
class PISA12RLong(Dataset):

    def __init__(self):
        # --- Response data ---
        resp = pd.read_csv('datafiles/PISA12R/cleaned.csv')
        ...
        # Item columns: strip 'item_' prefix; column position = item_idx
        item_cols = [c for c in resp.columns if c.startswith('item_')]
        self.item_names = [c[len('item_'):] for c in item_cols]
        self.n_users = len(self.student_ids)
        self.n_items = len(item_cols)

        self.Q = pd.read_csv('datafiles/PISA12R/Q.csv').to_numpy(copy=True)
        ...
        # --- Log data ---
        log = pd.read_csv('datafiles/PISA12R/log_selected.csv')

        # Combine event_name and event_type into a single event identifier.
        # When event_type is NaN (e.g. START_ITEM / END_ITEM), use event_name alone.
        log['event_combined'] = (
